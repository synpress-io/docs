...
The second argument is a function that will be used to unlock the wallet before each test. Currently, you have to pass the `unlockForFixture` function from `@synthetixio/synpress/playwright` which unlocks the MetaMask wallet, but soon you'll be able to pass here unlock functions from other wallets!
...
To access the MetaMask API, you must create an instance of the `MetaMask` class as shown above. To learn more about the constructor and all the methods this class provides, see [its API reference](../../api-reference/playwright/classes/MetaMask).
